...
public:
  ...
  void ROS() 
  {
    ...
    FRS_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/front_right_steering_controller/command", 10);
    FLS_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/front_left_steering_controller/command", 10);
    RRS_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/rear_right_steering_controller/command", 10);
    RLS_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/rear_left_steering_controller/command", 10);
    
    // Publishers for the speeds.
    FRW_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/front_right_wheel_controller/command", 10);
    FLW_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/front_left_wheel_controller/command", 10);
    RRW_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/rear_right_wheel_controller/command", 10);
    RLW_pub = nh_.advertise<std_msgs::Float64>(
      "/idris_sim/rear_left_wheel_controller/command", 10);
    
    // The subscriber for Ackermann messages
    ackermann_sub = nh_.subscribe("/idris_sim/cmd_ackermann", 10,
                                 &idris_driver::cmdAckermannCallback, this);
    
    vel_sub = nh_.subscribe("/idris_sim/cmd_vel", 10, 
                           &idris_driver::cmdVelCallback,
                           this);

    odom_sub = nh_.subscribe("/idris_sim/odom", 10,
                            &idris_driver::odomCallback,
                            this);

#ifndef POSCTRL
    // Subscriber for the joint values.
    jointValues_sub = nh_.subscribe("/idris_sim/joint_states", 100,
